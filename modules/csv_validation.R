...
# SECURE CSV Validation for SAV Quantitative Data
...
get_csv_error_message <- function(validation_result) {
  ...
  if (length(issues) > 0) {
    if (any(grepl("Missing required column", issues, ignore.case = TRUE))) {
      guidance <- paste0(
        "\n\n<strong>How to fix:</strong> Ensure your CSV contains these column names (map SAV Metric to stressor and Ecological Function to response):\n",
        "• curve.id\n",
        "• stressor.label\n",
        "• stressor.x\n",
        "• units.x\n",
      ...
      guidance <- "\n\n<strong>How to fix:</strong> Ensure stressor.x (SAV Metric) and response.y (Ecological Function) columns contain only numbers (no text or special characters except decimals, NA is allowed)"
      ...
      guidance <- sprintf(
        "\n\n<strong>How to fix:</strong> Each curve must have at least %d rows where BOTH stressor.x (SAV Metric) and response.y (Ecological Function) are non-NA numbers",
