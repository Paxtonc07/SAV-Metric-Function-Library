...
get_csv_error_message <- function(validation_result) {
  ...
  if (length(issues) > 0) {
    if (any(grepl("Missing required column", issues, ignore.case = TRUE))) {
      guidance <- paste0(
        "\n\n<strong>How to fix:</strong> Ensure your CSV contains these exactly named required columns (map SAV Metric to stressor and Function to response):\n",
        "• curve.id\n",
        "• stressor.label\n",
        "• stressor.x\n",
        "• units.x\n",
  ...
  tryCatch(
    {
      existing_title <- dbGetQuery(db_conn,
                                   "SELECT 1 FROM sav_records WHERE LOWER(TRIM(title)) = LOWER(TRIM($1)) LIMIT 1",
                                   params = list(title)
      )
      ...
      if (nrow(existing_title) > 0) {
        return(list(
          duplicate = TRUE,
          message = sprintf(
            "An SAV relationship with the title '%s' already exists in the database. Please use a different title to avoid duplication.",
            title
          )
        ))
      }
...
#'
#' Checks if the same metric-species-location combination already exists
#'
#' @param sav_metric Specific SAV metric
#' @param sav_species SAV Species name(s)
#' @param location Location value(s)
#' @param db_conn Database connection
#'
#' @return List with $conflict (logical) and $message (character)
check_data_conflict <- function(sav_metric, sav_species, location, db_conn) {
  if (is.null(sav_metric) || is.null(sav_species) || is.null(location)) {
    return(list(conflict = FALSE, message = ""))
  }
  
  metric_str <- paste(sav_metric, collapse = ", ")
  species_str <- paste(sav_species, collapse = ", ")
  location_str <- paste(location, collapse = ", ")
  
  tryCatch(
    {
      # Note: sav_species and location_waterbody are arrays in the new schema.
      # We cast them to text just for the sake of checking string similarity.
      similar <- dbGetQuery(db_conn,
                            "SELECT title, specific_sav_metric, sav_species::text, location_waterbody::text
         FROM sav_records
         WHERE specific_sav_metric = $1 AND sav_species::text ILIKE $2
         LIMIT 5",
                            params = list(metric_str, paste0("%", species_str, "%"))
      )
      ...
      if (nrow(similar) > 0) {
        ...
        return(list(
          conflict = TRUE,
          message = sprintf(
            "Warning: The combination of metric '%s' and species '%s' already exists in these records:\n\n%s\n\nYou can still proceed if this is a different study or updated data.",
            metric_str, species_str, titles
          )
        ))
      }
